...
- **埋め草バーの除外（実測に基づく確定事項）**: Dukascopy の日次ローソクファイルは
  休場時間帯（土曜全日・日曜のオープン前・金曜引け後・祝日）にも直前価格を複製した
  フラットバー（`volume == 0` かつ `high == low`、全体の約29.5%）を含む。実際の
  チャート（MT4/TradingView）には存在しないバーであり、SMA400/BB を歪めるため、
  **parquet 構築時に `volume == 0 かつ high == low` の行をすべて除外する**
  （ティックが1本も無い分は実チャートにもバーが立たない、と同じ挙動）
